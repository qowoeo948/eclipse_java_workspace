...
public class RegistForm extends JPanel{
	...
	Product product; //상품페이지 이벤트 구현하기 위해서
	...
	public RegistForm(Product product) {
		...
		//최상위 카테고리 채우기(db연동x, 기존product에 있는 탑카테고리 재사용하자)
//		for(int i=0;i<product.topList.size();i++) {
//			
//		}
	...
	//우리가 선택한 아이템으로 부터 subcategory의 pk를 가져오기
	...
	public int regist() {
		...
		try {
			pstmt=product.getAdminMain().getCon().prepareStatement(sql);
			pstmt.setInt(1, getSubId(ch_sub.getSelectedItem()));//선택한 아이템의 pk를 구하여 바인드변수에 대입
